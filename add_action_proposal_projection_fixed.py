...
def draw_action_proposals(image, boundary_points, start_point, number_size=15, navigability_mask=None, min_path_length=50, draw_degree=True, min_arrow_width=10):
    ...
    for i, point in enumerate(boundary_points):
        ...
        cv2.arrowedLine(
            ...
            entry_point,
            end_point,
            (255, 0, 0),  # Changed from (0, 255, 0) to (255, 0, 0) for red color
            2,  # Line thickness
            tipLength=0.03
        )
    ...
    text_ta_y = turn_point_center_y + text_0_size[1] // 2 # Align vertically with the "0"
    ...
    for i, details in enumerate(valid_points_details):
        action_number = i + 1 # Numbers start from 1
        mid_x = details['mid_x']
        mid_y = details['mid_y']
        ...
        cv2.circle(output_image, (mid_x, mid_y), number_size, (255, 255, 255), -1)  # Changed from (0, 255, 0) to (255, 255, 255) for white
        ...
        if draw_degree:
             ...
             cv2.putText(
                 ...
                 cv2.FONT_HERSHEY_SIMPLEX,
                 0.5,
                 (255, 0, 0),  # Changed from (0, 255, 0) to (255, 0, 0) for red
